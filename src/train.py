...
def train_tfidf(docs, n, out, name):
    ...
    #downsample/feature reduction for dictionary
    ...
    dictionary.filter_extremes(no_below=int(n*0.01), no_above=0.99)

    print('length of dict after filtering is {} ...'.format(len(dictionary)))
    print('creating bag of words for corpus and building tfidif model')
    #create tfidf
...
def main():
    ...
    conn = db.articles_database(args.db_path)
    if args.month == None:
        n = conn.get_count_of_articles_for_year(args.year)
        corpus = conn.select_articles_by_year(args.year)
        train_tfidf(list(iter(corpus)), n, args.output_path, '{}'.format(args.year))
    else:
        ...
        train_tfidf(list(iter(corpus)), n, args.output_path, '{}_{}'.format(args.year, args.month))
