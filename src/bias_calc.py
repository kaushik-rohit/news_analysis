...
def get_bigrams_for_year_and_month_by_clusters(db_path, dct, tfidf_model, year, month, group_by, bias_type,
    ...
    if bias_type == 'within_source':
        ...
        in_cluster, not_in_cluster, in_cluster_tomorrow = cluster_analysis.get_cluster_of_articles(db_path, dct,
                                                                                                   ...
                                                                                                   year, month,
                                                                                                   threshold)
        # helpers.save_cluster(in_cluster, not_in_cluster, in_cluster_tomorrow, '../results/clusters_{}_{}.csv'.format(
        #    year, month))
...
def bias_averaged_over_month(db_path, dct, tfidf_model, top1000_bigram, year, month, group_by, bias_type, std_type,
                             threshold=0.3):
    """
    Parameters
    ----------
...
def bias_averaged_over_year(db_path, dct, tfidf_model, top1000_bigram, year, group_by, bias_type, std_type,
    ...
    if bias_type == 'within_source':
        ...
        print('aggregating bigram share for above median cluster')
        aggregate_share_above_median2 = aggregate_bigrams_month_share(top_bigrams_share_by_month_above_median_in_cluster,
                                                                      total_bigrams_by_month_above_median_in_cluster,
                                                                      aggregate_source_count_above_median2,
                                                                      top_bigrams)

        print('aggregating bigram share for below median cluster')
        aggregate_share_below_median2 = aggregate_bigrams_month_share(top_bigrams_share_by_month_below_median_in_cluster,
                                                                      total_bigrams_by_month_below_median_in_cluster,
...
def main():
    ...
    if month is None:
        bias_averaged_over_year(db_path, dct, tfidf_model, top_1000_bigrams, year, group_by, bias_type, std_type,
                                threshold=threshold)
    else:
        bias_averaged_over_month(db_path, dct, tfidf_model, top_1000_bigrams, year, month, group_by, bias_type,
                                 std_type, threshold=threshold)
